;LOTR-reader-test. This is a new book.
;
; Works perfect.
; source text: 
; https://github.com/ganesh-k13/shell/tree/master/test_search/www.glozman.com/TextPages
; 
; improved output a bit, no more weird space lines, unless input is weird.
;
; no need for the upper memory on this one.
;
;
;
; NTASM bible reader v9
; This is the NT version. Found a cleaner source of text
; https://quod.lib.umich.edu/k/kjv/browse.html
; downloaded and cleaned up. Created new wordbanks and corrected the
; wordtable.
;
; source fixes in file partbib-noastrop.txt
;  removed hyphens (') and brackets [], the word "asss"
;  trims wordbanks, and overall textfiles
;  1580 bytes free at $c000
;  fixed the flashing cursor
;  
;
;Asm Bible Read v8
;
;  1318 bytes left in $c000 area
;
; todo 
;  maybe
;  book1-4 fits on 2 disks. side a and b. But will require the
;  upper file to be on side b, requiring the user to flip disk
;  after init read.
;  luckily book1 needs to be on side b.
;    side A: mainprogram, book2, book3
;    side B: upper, book1, book4
;
;
;
;Asm Bible Read v7
;
; todo 
;  optimize the word banks locations to get more ram at $c000
;
;61   b02.seq
;585  b03.seq
;2759 b04.seq
...
;squeeze banks into 8192 bytes, $e000-FFFF - 6 ? for vectors?
;best option
;bank5
;bank12
;bank13
;bank2
;leaves 6 bytes :D this is the new UPPER.PRG?
; hmm no, because its better to end somewhere low after $c000
; a better placement -- check squeezedata.bas
;00150  0801             ;OPTIMIZED -- TEST
;00151  0801             INCBIN     "B02.SEQ"
;00152  083E             INCBIN     "B05.SEQ"
;00153  1D83             INCBIN     "B06.SEQ"
;00154  3E0D             INCBIN     "B07.SEQ"
;00155  641E             INCBIN     "B08.SEQ"
;00156  876F             INCBIN     "B09.SEQ"
;00157  A617             INCBIN     "B10.SEQ"
;00158  BCC7             INCBIN     "B12.SEQ"
;00159  C3D8             INCBIN     "B15.SEQ"
;00160  C4B1 

;new upper
;00002  E000             INCBIN     "B03.SEQ"
;00003  E249             INCBIN     "B04.SEQ"
;00004  ED10             INCBIN     "B11.SEQ"
;00005  FAA1             INCBIN     "B13.SEQ"
;00006  FE08             INCBIN     "B14.SEQ"
;00007  FF79             INCBIN     "B16.SEQ"
;00008  FFCA 

;newtablo  byte $01,$00,$49,$3e,$83,$0d,$1e,$6f,$17,$10,$c7,$a1,$08,$d8,$79
;newtabhi  byte $08,$E0,$E2,$08,$1d,$3e,$64,$87,$a6,$ed,$bc,$fa,$fe,$c3,$ff
;
;  yes - DONE!
;  main program now ends at $cad6, before $cffb
;  leaving 1322 new bytes to play with :D
; 
; also fixed the bug with a space in the beginning of a line
; seems i had to optimize the chkcomma routine, and clear the
; lastspecial flag in checkcx. Perfect readout!
; this check is "overcoded", need to simplify ... later
;
;
;Asm Bible Read v6
;
...
bascx=$034d
bassp=$034e
basdstr=$034f             ;string, technically one char
basw=$0350
basdu=$0353               ;do case?
...
;*=$0801
*=$3267; shift data up, so we hit $c000; LOTR
...
;*=$C9D7 
;*=$C4B1 
;new optimized adress to start at $c4b1
               
        ;sei    ;dont disable interrupts, since IO needs them
;LOTR is under C000, force - no force
;*=$c000
        ; SETUP
...
showbookmenu            
        ...
        jsr $ffd2
        inx
        cpx lotrmenuend-lotrmenu
        bne showbookmenu
        jsr waitforkeynomenu
...
initvars
        ...
        lda #22         ;lines to print before pause
        sta linecnt
        
        ;we dont need an upperbank, all fits below $c000
...
;        sta bascx       ;cx=cx+2
        ...
        lda #1
        sta lastspecial
        jsr checkcx     ;this can reset lastspecial, which it should
        jmp readmorenext
...
;filename text "ntupper"
;fend

;LOTR new table
wtablo byte $67,$b8,$ec,$37,$2d,$2a,$a0,$c5,$b7,$57,$8d,$39,$88,$ea,$f5
wtabhi byte $32,$32,$34,$3e,$4e,$65,$7f,$97,$aa,$b5,$bb,$be,$bf,$bf,$bf

;LOTR table
;wtablo  byte $03,$52,$a2,$01,$35,$6e,$14,$b1,$e7,$eb,$5e,$20,$77,$e1,$f5
;wtabhi  byte $30,$30,$32,$3c,$4c,$63,$71,$96,$a9,$b4,$bb,$be,$bf,$bf,$bf

;new NT table
;wtablo  byte $01,$00,$92,$39,$f8,$09,$7a,$76,$49,$9f,$24,$40,$ec,$6a,$28
;wtabhi  byte $08,$e0,$e2,$08,$1d,$3f,$65,$88,$a6,$ed,$bc,$fa,$fc,$c2,$fe

;the new table of banks. Dont matter where in ram they are
;this is optimized, leaving more memory free at $c000 area
;wtablo  byte $01,$00,$49,$3e,$83,$0d,$1e,$6f,$17,$10,$c7,$a1,$08,$d8,$79
;wtabhi  byte $08,$E0,$E2,$08,$1d,$3e,$64,$87,$a6,$ed,$bc,$fa,$fe,$c3,$ff
...
;new bitlen for LOTR
wtabln byte 7,10,11,12,12,12,12,11,10,9,8,7,5,1,1

;bitlen of each bank - bible
...
CHARSET 2       ;directive for CBM studio to threat letters with case
                ;seems only to work on string with '', hmm. but works

               ;1234567890123456789012345678901234567890
menu      text "1=page 2=continuous 3=text color 4=more"
menu2     text "5=background 6=foreground 7=     8=quit"
intro     text 'LOTR64. Full Book. Code by Eyvind Ebsen'

flipdisk  text "flip disk"

lotrmenu  byte $0d,$0d,$0d      ;print a menu, max 255 chars
          text "Select a disk"
          byte $0d,$0d
          text "1. disk1 - part1"
          byte $0d
